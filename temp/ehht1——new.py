...
def generate_augmented_time_series_with_transformer(original_data_with_target, s=2, n=10, noise_std=0.01, seed=42):
    ...
    target_row = original_data_with_target[-1:, :] 
...
def main():
    # --- 用户配置部分 ---
    input_filepath = r"Time-Series-Library/dataset_refine/ETTh1.csv" # <--- 确保路径正确
    ...
    strength_levels_noise_std = np.linspace(0.005, 0.05, 10).tolist() 
    num_augmentations_per_strength_level = 5 
    s_param = 3
    seed_param = 42
    # --- 配置结束 ---
